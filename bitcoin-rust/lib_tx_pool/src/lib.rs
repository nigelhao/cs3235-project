...
mod tests {
    use lib_chain::block::{
        BlockNode, BlockNodeHeader, BlockTree, MerkleTree, Transaction, Transactions,
    };
    ...
    fn test_transaction_ops() {
        ...
        assert!(filtered_tx
            .iter()
            .any(|tx| tx.message == "SEND $300   // By Alice   // 1678173978750"));
        assert!(filtered_tx
            .iter()
            .any(|tx| tx.message == "SEND $100   // By Alice   // 1678173979751"));
        assert!(filtered_tx
            .iter()
            .any(|tx| tx.message == "SEND $200   // By Alice   // 1678173980752"));
        assert!(filtered_tx
            .iter()
            .any(|tx| tx.message == "SEND $100   // By Alice   // 1678173982754"));
    }
